...
from minigrid_gotobox import GoToBoxEnv


class BoxObsWrapper(ObservationWrapper):
        ...
        self.observation_space = Dict(
            {
                ...
                "box_color": Box(low=0.0, high=1.0, shape=(6,), dtype=np.float32),
            }
        )
    ...
    def observation(self, obs):
        wrapped_obs = {
            "image": obs["image"],
            "box_color": self.color_one_hot_dict[self.target_color],
        }

        return wrapped_obs


class BoxEnvExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: Dict):
        ...
        for key, subspace in observation_space.spaces.items():
            if key == "image":
                ...
                extractors["box_color"] = nn.Linear(subspace.shape[0], 32)
...
def main():
    ...
    policy_kwargs = dict(features_extractor_class=BoxEnvExtractor)
    ...
    if args.train:
        ...
        checkpoint_callback = CheckpointCallback(
            save_freq=1e5,
            save_path=f"./models/ppo/minigrid_gotobox_{stamp}/",
            name_prefix="iter",
        )
        ...
        model = PPO(
            ...
            verbose=1,
            tensorboard_log="./logs/ppo/minigrid_gotobox_tensorboard/",
        )
        model.learn(
            2e6,
            tb_log_name=f"{stamp}",
